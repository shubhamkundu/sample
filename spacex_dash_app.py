...
def get_pie_chart(selected_site):
    ...
    if (selected_site == 'ALL'):
        ...
        pie_data = df.groupby('class')['Launch Site'].count().reset_index().sort_values('class')
        fig = px.pie(
            pie_data,
            values='Launch Site',
            names='class',
...
def get_scatter_chart(selected_site, selected_payload_mass_range):
    scatter_data = spacex_df[
        spacex_df['Payload Mass (kg)'].between(
            selected_payload_mass_range[0],
            selected_payload_mass_range[1]
        )
    ]
    if selected_site != 'ALL':
